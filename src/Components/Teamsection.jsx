...
export const Team = () => {
    return (
        <section className="relative">
            <div className="absolute bg-[url('/src/assets/images/Shapes/shape-19.png')] bg-center bg-no-repeat bg-cover left-0 top-0 w-full h-full z-0"></div>
            <div className="flex flex-col items-center w-full">
                ...
                <div className="w-full flex  flex-wrap justify-center items-center lg:flex-nowrap  lg:justify-between gap-4">
                    {team.map(({ id, Name, Post, src }) => {
                        return (
                            <div key={id}  className="relative grow">
                                <div>  
                                    <img src={src} alt="img" className="rounded-3xl w-full h-full" />
                                </div>
                                    <div className="bg-[#161b2299] px-4 py-2 text-[#fff] hover:text-[#181e25] hover:bg-[#fff6] backdrop-blur-[20px] absolute  left-0 bottom-0 w-full rounded-b-3xl flex items-center justify-between transition-all duration-500">
                                        <div className="bg-['url(/src/assets/images/Shapes/shape-17.png')] inset-0 h-full  bg-cover bg-no-repeat bg-center left-0 top-0 -z-10">
                                            <h3 className='font-Outfit text-2xl'>{Name}</h3>
                                            <span className='text-sm text-font'>{Post}</span>
                                        </div>
                                        <div className=''>
                                            <FaPlusCircle size={30} className='text-[#fff]' />
                                            <ul className="bg-[url('/src/assets/images/Shapes/shape-18.png')] bg-cover bg-center bg-no-repeat absolute  p-3 flex flex-col gap-4 bottom-6 right-2 opacity-0  translate-y-2   hover:opacity-100 hover:scale-100 hover:translate-y-0 transition-all duration-500 ease-in-out z-20  cursor-pointer ">
